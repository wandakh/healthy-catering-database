...
(2, 'aulia rahma', 22, 'jl. dago no. 45, bandung', 'p', 50.00, 160.00, 'melihat-lihat');
...
insert into user_alergi (id_user, nama_alergi) values
(1, 'seafood');
...
select current_time();
call sp_buat_pesanan_otomatis_kurir(1, 3, 'malam', 1);
...
call sp_rekap_menu_diatas_rata_rata(1);
...
update user_biasa set status_user = 'nonaktif' where id_user = 1;

call sp_buat_pesanan_otomatis_kurir(1, 2, 'pagi', 1);
-- =========================================================================
-- tahap 3: struktur logika (stored procedure, trigger, dcl)
-- =========================================================================
...
begin
    ...
    select berat_badan, tinggi_badan, jenis_kelamin 
    into v_berat, v_tinggi, v_jk
    from user_biasa 
    where id_user = p_id_user;
    
    if v_berat is null or v_tinggi is null then
        signal sqlstate '45000'
        set message_text = 'gagal: data fisik belum lengkap untuk kalkulasi gizi!';
    end if;
    
    if v_jk = 'l' then 
        set v_kalori = (10 * v_berat) + (6.25 * v_tinggi) + 5;
    else
        set v_kalori = (10 * v_berat) + (6.25 * v_tinggi) - 161;
    end if;
    ...
    on duplicate key update 
        id_ahli_gizi = p_id_ahli_gizi, target_kalori = v_kalori, protein = v_protein, lemak = v_lemak, karbo = v_karbo;
end //
delimiter ;
...
begin
    ...
    select id_kurir into v_id_kurir
    from kurir
    where id_kurir not in (
        select id_kurir from detail_pengiriman where status_pengiriman = 'sedang diantar'
    )
    limit 1;
    ...
    if v_id_kurir is not null then
        ...
        signal sqlstate '45000' set message_text = 'maaf, semua armada kurir di slot ini sedang sibuk bertugas!';
    end if;
end //
delimiter ;
...
begin
    ...
    select count(*), max(j.tanggal_hari) into v_cek_id, v_tanggal_kirim
...
begin
    ...
    if v_target_kalori is null then
        ...
        elseif v_selisih > 150 then
            set v_status = concat('⚠️ KURANG GIZI! Kalori di tanggal ini kurang ', v_selisih, ' kkal dari target.');
...
begin
    ...
    if v_cek_target = 0 then
        ...
        where t.id_user = p_id_user
          -- ATURAN 1: Filter AMAN dari bahan pemicu alergi user
...
begin
    ...
    from db_catering_sehat.resep_menu r
    join db_catering_sehat.stok_bahan s on r.id_stok = s.id_stok
...
for each row
begin
    update db_catering_sehat.stok_bahan s
    join db_catering_sehat.resep_menu r on s.id_stok = r.id_stok
    set 
...
-- -------------------------------------------------------------------------
-- -------------------------------------------------------------------------
-- c. data control language (dcl) - final & sinkron
-- -------------------------------------------------------------------------

-- 1. Admin Utama (Punya Kuasa Penuh, termasuk untuk sistem backend)
create user if not exists 'admin_utama'@'localhost' identified by 'passwordadminutama123!';
grant all privileges on db_catering_sehat.* to 'admin_utama'@'localhost';

-- 2. Kurir (Akses logistik & konfirmasi SP)
create user if not exists 'kurir'@'localhost' identified by 'kurirkateringsehat2026!';
grant select, update on db_catering_sehat.jadwal_pengiriman to 'kurir'@'localhost';
grant select, update on db_catering_sehat.detail_pengiriman to 'kurir'@'localhost';
grant execute on procedure db_catering_sehat.sp_konfirmasi_makanan_diterima to 'kurir'@'localhost';

-- 3. Staf Gizi (Akses master data user & kalkulator SP gizi)
create user if not exists 'staf_gizi'@'localhost' identified by 'ahligizi2026!';
grant select on db_catering_sehat.user_biasa to 'staf_gizi'@'localhost';
grant select on db_catering_sehat.user_alergi to 'staf_gizi'@'localhost';
grant select, insert, update on db_catering_sehat.target_gizi to 'staf_gizi'@'localhost';
grant execute on procedure db_catering_sehat.sp_hitung_gizi_otomatis to 'staf_gizi'@'localhost';
grant execute on procedure db_catering_sehat.sp_rekomendasi_menu_aman to 'staf_gizi'@'localhost';
grant execute on procedure db_catering_sehat.sp_cek_pemenuhan_gizi_harian to 'staf_gizi'@'localhost';

-- 4. Staf Dapur (Akses produksi & inventaris stok)
create user if not exists 'staf_dapur'@'localhost' identified by 'dapurkatering2026!';
grant select on db_catering_sehat.pemesanan to 'staf_dapur'@'localhost';
grant insert, update on db_catering_sehat.menu to 'staf_dapur'@'localhost';
grant select, update on db_catering_sehat.stok_bahan to 'staf_dapur'@'localhost';
grant select, insert, update, delete on db_catering_sehat.resep_menu to 'staf_dapur'@'localhost';

-- 5. Staf Keuangan (Akses audit pendapatan & nota SP)
create user if not exists 'staf_keuangan'@'localhost' identified by 'uangmasuk2026!';
grant select on db_catering_sehat.pemesanan to 'staf_keuangan'@'localhost';
grant select on db_catering_sehat.user_biasa to 'staf_keuangan'@'localhost';
grant execute on procedure db_catering_sehat.sp_cek_nota_pelanggan to 'staf_keuangan'@'localhost';
grant execute on procedure db_catering_sehat.sp_rekap_menu_diatas_rata_rata to 'staf_keuangan'@'localhost';

-- Terapkan semua hak akses baru
flush privileges;